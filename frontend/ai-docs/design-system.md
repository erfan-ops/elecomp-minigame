...
lines) → `ChoiceGrid` (step 1 renders five cards — the skip option is the full-width last one) →
`NavButtons`.

Survey logic is two local steps inside `SurveyPage` (the SURVEY phase is unchanged):
step 1 = four count-range cards + the full-width skip card, step 2 = بله/خیر. The ranges map to
stored counts
...
| 13 | Page 2: the «در سازمان یا شرکتی کار نمی‌کنم» skip option is not in the Figma frame but must stay (existing behavior, HARD RULES). Since 2026-08-29 it is the fifth card in step 1's `ChoiceGrid` (full width, `.choice-card--wide`) instead of a separate checkbox pill — all five choices are mutually exclusive, so they share one single-select grid. | HARD RULES |
