...
|---|---|---|---|---|---|---|
| `RegistrationPage` | `src/pages/RegistrationPage.tsx` | Collect mobile; validate; anti-replay check; `register` | none | local `mobileDigits`, `error`, `checking`, `topEntries`, `stats`; session `register` | `resultRepository.getResults()` twice: once on mount for the leaderboard panel's top 5 **and** the stats panel (`buildGameStats` — one fetch, both panels), once per submit for anti-replay | Digit cap 10 hard-coded in `appendDigit`; validation via `isValidMobileDigits`. **Redesigned page 1**: container logic unchanged, presentation composed from `src/components/ui/` components inside `PageShell` — since 2026-08-26 the **same shell + `GameHeader` + `FloatingDecorations` as pages 2+** (no more `Container.svg` logo); since 2026-08-31 the «آمار مسابقه» `StatsPanel` fills the page's bottom space |
| `SurveyPage` | `src/pages/SurveyPage.tsx` | Collect `employeeCount` + `hasBenefits`, or skip | none | local `step` (1 \| 2), `countChoice`, `hasBenefits`, `notEmployed`; session `completeSurvey`, `startNewUser` | none | **Redesigned page 2**: two local steps inside the single SURVEY phase — step 1 = a five-option single-select grid (`STEP_ONE_OPTIONS` = the four count ranges `COUNT_OPTIONS` → `COUNT_TO_EMPLOYEES` {10,50,300,301}, plus «در سازمان یا شرکتی کار نمی‌کنم» as a full-width fifth card), step 2 = بله/خیر. The skip card stores `{0, false}`. بازگشت: step 2 → step 1; step 1 → `startNewUser()`. ادامه disabled until the step is answerable |
...
| `PageShell` | `src/components/ui/PageShell.tsx` | Redesigned page frame: the fixed 1080×1800 design canvas (`--ds-canvas-*`), corner glows, logo, content frame, Almas credit footer pinned to the canvas bottom — scaled by `--s` and centered by `.app` | `children` | none | none | See `design-system.md` |
...
| `StatsPanel` | `src/components/ui/StatsPanel.tsx` | «آمار مسابقه» panel (page 1, bottom): 5 tiles — total prize paid out (gold value + «تومان»), distinct players, winners with exactly 1/2/3 correct digits | `stats: GameStats` | none | none | Tiles built from `buildGameStats(results)`; digit badges wear the primary gradient circle, emoji badges a subtle gold one |
| `GameHeader` | `src/components/ui/GameHeader.tsx` | Shared page header (every page): Smartis logo (`public/smartis_logo.svg`) right of the centered tagline «تجربه هیجان در غرفه اسمارتیز» (Vazirmatn 600, letter-spacing 0) | none | none | none | Vazirmatn stack (`--ds-font-body`) |
| `FloatingDecorations` | `src/components/ui/FloatingDecorations.tsx` | Atmospheric decoration layer (every page) | none | none | none | 8 `public/deco/*.svg` files as `<img>` at design-px positions (relative `src` so the pywebview `file://` load resolves them), `aria-hidden`, `pointer-events: none`; each plays its own CSS motion (bob / sway / 3D flip / twinkle / drift / hover / bounce / spin) staggered by negative `animation-delay`s |
| `ChoiceGrid` | `src/components/ui/ChoiceGrid.tsx` | 2-column glass answer cards | `options`, `selected`, `onSelect`, `wideLastOption?` | none (controlled) | none | Generic over the option type; `aria-pressed` per card; `wideLastOption` spans the final card across both columns (survey step 1's skip option) |
...
| `RegistrationPage` leaderboard + stats load | `try/catch` → `topEntries: []` and `stats: EMPTY_GAME_STATS` (both panels degrade silently to their empty states — zeros; no retry UI). |
...
- All interactive elements are real `<button type="button">`. There are no `div` click handlers on
  controls.
- `aria-pressed` on category cards, the survey choice cards (including step 1's full-width skip
  card), and the keypad.
