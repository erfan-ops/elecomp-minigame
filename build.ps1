...
aside and restored afterwards, because PyInstaller's -y wipes that folder.

Keep this file pure ASCII. Windows PowerShell 5.1 reads .ps1 as ANSI unless the
file has a UTF-8 BOM, so a UTF-8 em dash decodes to a curly quote that
PowerShell treats as a string terminator - which breaks the parse.
#>
...
# $ErrorActionPreference does not apply to native executables, so check by hand.
...
$logFile = Join-Path $ExeDistDir "pywebview.log"
Write-Host ""
